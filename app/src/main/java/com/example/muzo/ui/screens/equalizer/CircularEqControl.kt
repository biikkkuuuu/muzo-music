...
/**
 * Echo-Music style Axion Circular Equalizer Touch Dial.
 * Features 3 interactive radial axes for Bass, Mid, and Treble with
 * morphing vector wave geometry and real-time haptic touch tracking.
 */
@Composable
fun CircularEqControl(
    ...
    Canvas(
        ...
        drawPath(wavePath, primaryContainer.copy(alpha = 0.15f))
        drawPath(wavePath, primary.copy(alpha = 0.55f), style = Stroke(2.5f, cap = StrokeCap.Round))
        ...
        for (i in 0..2) {
            ...
            drawCircle(primary.copy(alpha = if (isActive) 0.3f else 0.15f), if (isActive) 34f else 26f, points[i])
